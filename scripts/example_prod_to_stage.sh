...
readonly STAGE_HOST=deploy@stage.savima.org
readonly PROD_HOST=deploy@savima.org
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/savima/shared/media
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres savima -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d savima -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/savima/shared/venv/bin/activate && cd /mnt/persist/www/savima/current/src &&"

ssh $STAGE_HOST "$manage_prefix python manage.py change_site_domain --site_id=1 --new_site_domain='stage.savima.org'"
